# Generated by Powerlevel10k configuration wizard on 2024-09-07 at 13:01 CEST.
...
# 24h time, angled separators, sharp heads, round tails, 2 lines, dotted, right frame,
...
() {
  ...
  # typeset -g POWERLEVEL9K_MULTILINE_FIRST_PROMPT_SUFFIX='%234F─╮'
  # typeset -g POWERLEVEL9K_MULTILINE_NEWLINE_PROMPT_SUFFIX='%234F─┤'
  # typeset -g POWERLEVEL9K_MULTILINE_LAST_PROMPT_SUFFIX='%234F─╯'

  # Filler between left and right prompt on the first prompt line. You can set it to ' ', '·' or
  # '─'. The last two make it easier to see the alignment between left and right prompt and to
  # separate prompt from command output. You might want to set POWERLEVEL9K_PROMPT_ADD_NEWLINE=false
  # for more compact prompt if using this option.
  typeset -g POWERLEVEL9K_MULTILINE_FIRST_PROMPT_GAP_CHAR='_'
  ...
  function my_git_formatter() {
    ...
    if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
      ...
      (( VCS_STATUS_COMMITS_BEHIND )) && res+="%B ${clean}⇣${VCS_STATUS_COMMITS_BEHIND}%b"
      # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
      (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
      (( VCS_STATUS_COMMITS_AHEAD  )) && res+="%B${clean}⇡${VCS_STATUS_COMMITS_AHEAD}%b"
    ...
    # *42 if have stashes.
    (( VCS_STATUS_STASHES        )) && res+="%B ${clean}*${VCS_STATUS_STASHES}%b"
    # 'merge' if the repo is in an unusual state.
    [[ -n $VCS_STATUS_ACTION     ]] && res+="%B ${conflicted}${VCS_STATUS_ACTION}%b"
    ...
    (( VCS_STATUS_NUM_UNTRACKED  )) && res+="%B ${untracked}${(g::)POWERLEVEL9K_VCS_UNTRACKED_ICON}${VCS_STATUS_NUM_UNTRACKED}%b"
    # "─" if the number of unstaged files is unknown. This can happen due to
    # POWERLEVEL9K_VCS_MAX_INDEX_SIZE_DIRTY (see below) being set to a non-negative number lower
    # than the number of files in the Git index, or due to bash.showDirtyState being set to false
    # in the repository config. The number of staged and untracked files may also be unknown
    # in this case.
    (( VCS_STATUS_HAS_UNSTAGED == -1 )) && res+="%B ${modified}─%b"
